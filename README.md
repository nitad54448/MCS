...
While the algorithm internally uses **coded units** (scaled from -1 to +1) to ensure mathematical stability during matrix inversion, the final statistics table presents the coefficients converted back to their **actual physical units** (e.g., °C, minutes). This directly changes how the model parameters are interpreted:

* **Intercept ($b_0$):** Represents the theoretical predicted response when all physical parameters are exactly zero (e.g., 0°C, 0 min). *Note: Because 0 is usually far outside the tested experimental domain, this value acts as a mathematical anchor for the regression plane rather than a practically achievable baseline.*
* **Linear Coefficients ($b_i$):** Represent the expected change in the response per 1 actual unit increase of that specific factor (e.g., the change in yield per +1°C or +1 min), assuming all other variables are held constant at zero. 
* **Quadratic Coefficients ($b_{ii}$):** Indicate the **curvature** of the response surface, scaled to the square of the actual physical units. A significant negative coefficient suggests a "peak" (maximum), while a positive one suggests a "valley" (minimum).
* **Interaction Coefficients ($b_{ij}$):** Describe the synergistic or antagonistic effects between two factors. They represent how the effect of one factor changes per 1 actual unit increase in the other factor.
