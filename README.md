...
* **Intercept ($b_0$):** Represents the theoretical predicted response when all physical parameters are exactly zero (e.g., 0°C, 0 min). *Note: Because 0 might be outside the tested experimental domain, this value acts as a mathematical anchor for the regression plane rather than a practically achievable baseline.*
